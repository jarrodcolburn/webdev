...
typedef FetchResponse = web.Response;
typedef FetchOptions = web.RequestInit;

Future<FetchResponse> fetchRequest(String resourceUrl) async {
  try {
    final options = FetchOptions(method: 'GET', credentials: 'include');
    final response = await web.window.fetch(resourceUrl.toJS, options).toDart;
    // final response = await promiseToFuture(_nativeJsFetch(resourceUrl, options));
    final body = response.body;
    // final body = await promiseToFuture(js_util.callMethod(response, 'text', []));
